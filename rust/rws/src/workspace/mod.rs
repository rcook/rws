...
impl Workspace {
    ...
    fn traverse(config_path: PathBuf, root_dir: PathBuf) -> std::io::Result<Workspace> {
        ...
        match (&dependencies_hash_opt, &dependency_command_hash_opt) {
            (Some(_), Some(_)) => panic!("Invalid: cannot specify both dependencies and dependency-command"),
            ...
            (None, None) => panic!("Invalid: must specify one of dependencies or dependency-command")
        }
    }
    ...
    fn traverse_with_dependencies(
        ...
        Workspace::traverse_helper(config_path, &root_dir, excluded_project_dirs, |project_dir| {
            ...
            match dependency_command_hash.as_vec(project_name.to_str().unwrap()) {
                Some(v) => (0..v.len()).into_iter().map(|i| String::from(root_dir.join(v.as_str(i).unwrap()).to_str().unwrap())).collect(),
                None => Vec::new()
            }
        })
    }
    ...
    fn traverse_with_dependency_command(
        ...
        Workspace::traverse_helper(config_path, &root_dir, excluded_project_dirs, |project_dir|
            get_deps(&project_dir, &dependency_command).unwrap()
                                .into_iter()
                                .map(|x| String::from(root_dir.join(x).to_str().unwrap()))
                                .collect::<Vec<_>>())
    }

    fn get_project_dirs_alpha(root_dir: &Path, excluded_project_dirs: &HashSet<PathBuf>) -> std::io::Result<Vec<PathBuf>> {
    ...
    fn topo_sort_project_dirs<F>(project_dirs_alpha: &Vec<PathBuf>, f: F) -> Vec<PathBuf> where F: Fn(&Path) -> Vec<String> {
    ...
    fn traverse_helper<F>(config_path: PathBuf, root_dir: &PathBuf, excluded_project_dirs: &HashSet<PathBuf>, f: F)
        -> std::io::Result<Workspace> where F: Fn(&Path) -> Vec<String> {
        let project_dirs_alpha = Workspace::get_project_dirs_alpha(&root_dir, &excluded_project_dirs).unwrap();
