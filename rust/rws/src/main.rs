...
fn main() -> std::io::Result<()> {
    ...
    let app = App::new("Richard's Workspace Tool")
        ...
        .subcommand(SubCommand::with_name(GIT_SUBCOMMAND)
        ...
        .subcommand(SubCommand::with_name(RUN_SUBCOMMAND)
    ...
    match matches.subcommand() {
        (GIT_SUBCOMMAND, Some(_)) => {
            ...
            do_git(&submatches.values_of("cmd").map(|x| x.collect()).unwrap_or(Vec::new()))
        },
        (INFO_SUBCOMMAND, Some(_)) => do_info(),
        (RUN_SUBCOMMAND, Some(_)) => {
            let submatches = matches.subcommand_matches(RUN_SUBCOMMAND).unwrap();
            do_run(&submatches.values_of("cmd").map(|x| x.collect()).unwrap_or(Vec::new()))
        },
...
fn do_info() -> std::io::Result<()> {
    ...
    println!("Workspace directory: {}", workspace.root_dir.to_str().unwrap().cyan());
    println!("Workspace configuration: file {}", workspace.config_path.to_str().unwrap().cyan());
...
fn run_helper<F>(f: F) -> std::io::Result<()> where F: Fn() -> Command {
    let current_dir = env::current_dir()?;
    let workspace = Workspace::find(&current_dir).unwrap();
